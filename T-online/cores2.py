...
sys.path.append("../..")
from utils import *
Rs = {'Abilene': 12, 'GEANT': 12, 'CERNET': 12}
Ws = {'Abilene': 2016, 'GEANT': 672, 'CERNET': 2016}


def evaluate(dataset_name, ed, fraction, mu, sigma, R, distribution="Gaussian", SNR=None, init="random"):
    # Run BayesCP
    Y, outliers_p = generator2(dataset_name, fraction, mu, sigma, distribution=distribution, SNR=SNR)
    if ed == None:
        ed = Y.shape[2]
    Y = Y[:, :, 0:ed]
...
def eval_SNR(dataset_name, distribution="Gaussian", init="random"):
    ...
    with open(os.path.join(os.path.join('../../results', dataset_name), 'T-online/SNR2_'+distribution+'_'+init+'_TPRS.json'), 'w') as FD:
        FD.write(json.dumps(TPRS))
    with open(os.path.join(os.path.join('../../results', dataset_name), 'T-online/SNR2_'+distribution+'_'+init+'_FPRS.json'), 'w') as FD:
        FD.write(json.dumps(FPRS))