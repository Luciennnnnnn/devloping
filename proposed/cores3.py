...
def eval_ratio(dataset_name, distribution="Gaussian", init='rand'):
    ...
    for fraction in range(1, 11, 1):
        model = evaluate(dataset_name, 8000, fraction / 100, 0, 0.1, Rs[dataset_name], distribution=distribution, init=init)
        TPRS.append(model['precision'])
        FPRS.append(model['FPR'])
        # if fraction == 10:
    ...
    with open(os.path.join(os.path.join('../../results', dataset_name), 'proposed/ratio_'+'impro'+'_TPRS.json'), 'w') as FD:
        FD.write(json.dumps(TPRS))
    with open(os.path.join(os.path.join('../../results', dataset_name), 'proposed/ratio_'+'impro'+'_FPRS.json'), 'w') as FD:
        FD.write(json.dumps(FPRS))
