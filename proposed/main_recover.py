...
if __name__ == '__main__':
    ...
    sigma = 0.1
    SNR = None
    parameters = {"ed": ed, "R":R, "K":K, "mu":mu, "sigma":sigma, "theta": args.theta, "fraction": 0, 
                "SNR":SNR, "noise_scheme": args.noise_scheme, 'outliers_scheme': args.outliers_scheme}
    
    logging.info('recover --- ed: %d, R: %d, K: %d, mu: %d, sigma: %f, theta: %f, noise_scheme: %s, outliers_scheme: %s'%(ed, R, K, mu, sigma, args.theta, args.noise_scheme, args.outliers_scheme))     
    start = time.time()
